...
(function () {
    ...
    viewOptions = ['autofetch', 'buttons', 'colModel', 'container',
        ...
        'actionplus', 'rows', 'rowList', 'selectedRows', 'tableID',
        'subgrid', 'subgridAccordion', 'onRowClick', 'onRowDblClick',
        'onReady', 'seguridad', 'onBeforeRender', 'afterRender', 'onRowExpanded',
        'onRowCollapsed', 'events', 'editar', 'detalle',
        'borrar', 'actionpicture', 'clone', 'prodDetalle'];
    ...
    _.extend(bbGrid.View.prototype, Backbone.View.prototype, {
        ...
        EventHandler: function (eventName, option1, option2, options) {
            ...
            switch (eventName) {
                case 'selected':
                    if (this.subgrid) {
                        // Ahora esta madre solo se activa cuando le das en el simbolo de +
                        if (options.target.classList[1] === 'fa-plus') {
                            //Evitar que despliegue el subgrid en caso de que le de click en el boton de menu accion
                            if (!(options.target.classList[3] === 'dropdown-toggle' ||
                              options.target.classList[0] === 'caret' ||
                              options.target.tagName === 'A'
                            )) {
                ...
                case 'sort':
                    this.onSort(option1);
                    if (this.afterRender && getType.toString.call(this.afterRender) === '[object Function]')
                        this.afterRender();
                    break;
                case 'checkall':
        ...
        render: function () {
            ...
            if (!this.$grid) {
                this.$grid = $('<table class="bbGrid-grid table table-curved table-condensed" ' + (this.tableID ? 'id="' + this.tableID + '"' : '') + ' />');
                if (this.caption) {
        ...
        showCollection: function (collection, allrows) {
            ...
            if (collection.length === 0 && !this.autofetch) {
                this.$grid.append('<tbody><tr class="bbGrid-noRows"><td colspan="' + this.colLength + '" style="text-align:center; font-weight:bold;">' + this.dict.noData + '</td></tr></tbody>');
            }
        ...
        toggleSubgridRow: function (model, $el, options) {
            var View, colspan, subgridRow, subgridContainerHtml,icon;
            options = options || {};
            ...
            //Validar si tiene el +
            if (options.target.classList.length === 3) icon = 'fa-minus';
            else icon = 'fa-plus';
            if (icon === 'fa-plus') {
                if (View.$subgridContainer) {
                    //Se comentó para que no se destruyera el sub-grid cad vez que se seleccionaba otro
                ...
                colspan = this.multiselect ? 2 : 0;
                subgridRow = _.template('<tr class="bbGrid-subgrid-row sub-' +  model.id + '"><td colspan="<%=extra%>"/><td colspan="<%=colspan %>"></td></tr>', null, templateSettings);
            ...
            {
                $('td.bbGrid-subgrid-control i', $el).removeClass('fa-minus');                
                subgridRow = $('.sub-' + model.id);
                $('.sub-' + model.id).remove();
                View.$subgridContainer = $(subgridRow);
                $('td.bbGrid-subgrid-control i', View.$subgridContainer).removeClass('fa-minus');
                if (View.$subgridContainer) {
                    View.$subgridContainer.remove();
                    delete View.$subgridContainer;
                }            
        ...
        renderRow: function (model) {
            var rowNum = _.size(this.rowViews);
            if (this.rows === rowNum) {
                return false;
            }

            this.rowViews[model.id] = new bbGrid.RowView({ model: model, view: this });    
            this.$grid.append(this.rowViews[model.id].render(rowNum).el);
        },
    ...
    _.extend(bbGrid.RowView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            ...
            <%} if (isActionEnable) {%>\
                <td style="overflow:visible; text-align:center; vertical-align:center;">\
                    <div class="btn-group">\
                        <button class="btn btn-outline btn-default dropdown-toggle btn-xs" data-toggle="dropdown" data-rowNum="<%=rowNumber%>"> Acción <span class="caret"></span></button>\
                        <ul class="dropdown-menu" data-modelid="<%=modelId%>">\
                            <% if (isProdDetalle === true) {%>\
                            <li><a href="javascript:;" class="accrowProdDetalle"><i class="fa fa-list-alt"></i>  Nueva actividad</a></li>\
                            <%} if (isEditar === undefined || isEditar === true) {%>\
                            <li><a href="javascript:;" class="accrowEdit"><i class="fa fa-edit"></i>  Editar</a></li>\
                            ...
                            <li><a href="javascript:;" class="accrowBorrar"><i class="fa fa-remove"></i>  Eliminar</a></li>\
                            <% if (isPicture === true) {%>\
                                <li class="divider"></li>\
                                <li><a href="javascript:;" class="accrowImage"><i class="fa fa-picture"></i>  Imagen</a></li>\
                                <%}%>\
                            <%} if (isActionPlus) {%>\
                                <li><a href="javascript:;" class="accrowCorreo"><i class="fa fa-envelope"></i>  Email</a></li>\
                            <%}%>\
                        </ul>\
                    </div>\
                </td>\
            <%} if (isContainSubgrid) {%>\
                <td class="bbGrid-subgrid-control">\
                    <i class="fa fa-plus<%if (isSelected) {%> fa-minus<%}%>">\
                </td>\
            <%} _.each(values, function (row) {%>\
                <td <%if (row.customclass) {%>class="<%=row.customclass%>"<%}%>  <% if (row.textalign === true) {%>style="text-align:center;"<%}%>  <% if (row.name === "bbGrid-actions-cell") {%>class="bbGrid-actions-cell"<%}%>>\
        ...
        render: function (rowNum) {
            ...
            html = this.template({
                ...
                isProdDetalle: this.view.prodDetalle,
    ...
    _.extend(bbGrid.TheadView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            ...
            <%} if (isContainSubgrid) {%>\
                <th style="width:15px"/>\
                <%} _.each(cols, function (col) {%>\
                    <th <%if (col.customclass) {%>class="<%=col.customclass%>"<%}%>  <%if (col.width) {%>style="width:<%=col.width%>"<%}%>><%=col.title%><i <% \
                    ...
                    <ul class="dropdown-menu pull-right">\
                        <% _.each(cols, function (col, index) {%>\
                            <li <% if (index === searchOptionIndex) { %>class="active"<% } %>>\
                                <a name="<%=index%>" href="javascript:;"><%=col.title%></a>\
                            </li>\
                        <%})%>\
                    </ul>\
                </div>\
    ...
    _.extend(bbGrid.FilterView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            ...
            <% } %>\
            <%_.each(cols, function (col) {%>\
                <td <%if (col.customclass) {%>class="<%=col.customclass%>"<%}%>>\
                    <%if (col.filter) {%>\
        ...
        filter: function (collection, options) {
            ...
            if (text.length > 0) {
                collection.reset(_.filter(collection.models, function (model) {
                    ...
                    if (option) {
                        /*Personalizado: 18.09.2017.. Sólo para la columna Genero agregarle exactamente igual
                        10.10.2017: Se agregó nombreGenero como opción de búsqueda*/
                        if (key === 'genero' || key === 'nombreGenero')
