...
var Indicador3 = {
    ...
    Eventos: function () {
        ...
        $(document).on('click', '#btnFilter', function () {
            Indicador3.CargaGrid();
            Indicador3.CargaGridRegionZona();
            Indicador3.CargaGridRegionZonaActividad();
            Indicador3.CargaGridRegionZonaActividadFigura();
            Indicador3.GraficoRegion();
            Indicador3.GraficoZona();
        });
    ...
    onExportarExcel: function () {
        var url = contextPath + "Indicadores/ExportIndReincidentesExcel?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador

        window.open(url, '_blank');
    },
    onExportarPDF: function () {
        var url = contextPath + "Pdf/ExportIndicadorValidacionPDF?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador
    ...
    CargaGrid: function () {
        ...
        $('#bbGrid-clear')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url = contextPath + "Indicadores/CargarIndFigurasReincidentesRegion?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador
        $.getJSON(url, function (data) {
            if (data.Success === false) { FCH.DespliegaError(data.Message); return; }
            $('#bbGrid-clear')[0].innerHTML = "";
            Indicador3.colRegion = new Backbone.Collection(data.datos);
            var bolFilter = Indicador3.colRegion.length > 0 ? true : false;
            if (bolFilter) {
    ...
    CargaGridRegionZona: function () {
        ...
        $('#indicadorZona')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url = contextPath + "Indicadores/CargarIndFigurasReincidentesRegionZona?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador
        $.getJSON(url, function (data) {
            if (data.Success === false) { FCH.DespliegaError(data.Message); return; }
            $('#indicadorZona')[0].innerHTML = "";
            Indicador3.colRegionZona = new Backbone.Collection(data.datos);
            var bolFilter = Indicador3.colRegionZona.length > 0 ? true : false;
            if (bolFilter) {
                var table = data.datos;
                var tabIndicador = "<table class='table table-striped' style='-webkit-box-shadow: 0px 0px 22px -2px rgba(0,0,0,0.75); -moz-box-shadow: 0px 0px 22px -2px rgba(0, 0, 0, 0.75);box-shadow: 0px 0px 22px -2px rgba(0, 0, 0, 0.75);border-collapse: collapse;-webkit-border-radius: 7px;-webkit-border-bottom-right-radius: 4px;-webkit-border-bottom-left-radius: 4px;-moz-border-radius: 7px;-moz-border-radius-bottomright: 4px;-moz-border-radius-bottomleft: 4px;border-radius: 7px;border-bottom-right-radius: 4px;border-bottom-left-radius: 4px;overflow: hidden;'>";
    ...
    CargaGridRegionZonaActividad: function () {
        ...
        $('#indicadorZonaActividad')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url = contextPath + "Indicadores/CargarIndFigurasReincidentesRegionZonaFigNom?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador
        $.getJSON(url, function (data) {
            if (data.Success === false) { FCH.DespliegaError(data.Message); return; }
            $('#indicadorZonaActividad')[0].innerHTML = "";
            Indicador3.colRegionZonaActividad = new Backbone.Collection(data.datos);
            var bolFilter = Indicador3.colRegionZonaActividad.length > 0 ? true : false;
            if (bolFilter) {
                var table = data.datos;
                var tabIndicador = "<table class='table table-striped' style='-webkit-box-shadow: 0px 0px 22px -2px rgba(0,0,0,0.75); -moz-box-shadow: 0px 0px 22px -2px rgba(0, 0, 0, 0.75);box-shadow: 0px 0px 22px -2px rgba(0, 0, 0, 0.75);border-collapse: collapse;-webkit-border-radius: 7px;-webkit-border-bottom-right-radius: 4px;-webkit-border-bottom-left-radius: 4px;-moz-border-radius: 7px;-moz-border-radius-bottomright: 4px;-moz-border-radius-bottomleft: 4px;border-radius: 7px;border-bottom-right-radius: 4px;border-bottom-left-radius: 4px;overflow: hidden;'>";
    ...
    CargaGridRegionZonaActividadFigura: function () {
        ...
        $('#indicadorRegionZonaActividad')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url = contextPath + "Indicadores/CargarIndFigurasReincidentesRegionZonaFigNomAct?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser; // El url del controlador
        $.getJSON(url, function (data) {
            if (data.Success === false) { FCH.DespliegaError(data.Message); return; }
            $('#indicadorRegionZonaActividad')[0].innerHTML = "";
            Indicador3.colRegionZona = new Backbone.Collection(data.datos);
            ...
            if (bolFilter) {
                ...
                for (j = 0; j < table.length; j++) {
                    ...
                    tabIndicador += "<td>" + table[j]['ComentariosNoValidacion'] + "</td>";
                    tabIndicador += "</tr>";
                }
    ...
    GraficoRegion: function () {
        ...
        $('#grafico1')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url1 = contextPath + "Indicadores/CargarIndFigurasReincidentesRegionGrafico?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser;    
            ...
            if (bolFilterFobGC) {
                ...
                FusionCharts.ready(function () {
                    var ChartFobGC = new FusionCharts({
                        ...
                        dataSource: {
                            "chart": {
                                "caption": "% Actividades rechazadas durante la validación por región",
                                "subCaption": "",
                                "captionFontSize": "14",
    ...
    GraficoZona: function () {
        ...
        $('#graficozona')[0].innerHTML = "<div class='alert alert-warning'>" +
            ...
            "<span>Cargando...</span>" +
            "</div>";
        var url1 = contextPath + "Indicadores/CargarIndFigurasReincidentesRegionZonaGrafico?idPeriodo=" + $('#selPeriodosMul').val() + "&Region=" + $('#selRegionesMul').val() + "&Zonas=" + $('#selZonasMul').val() + "&idUsuario=" + localStorage.idUser;
        $.getJSON(url1, function (data) {
            if (data.Success !== undefined) {
                FCH.DespliegaError(data.Message);
            ...
            if (bolFilterFobGC) {
                ...
                FusionCharts.ready(function () {
                    var ChartFobGC = new FusionCharts({
                        ...
                        dataSource: {
                            "chart": {
                                "caption": "% Actividades rechazadas durante la validación por zona",
                                "subCaption": "",
                                "captionFontSize": "14",
                                "subcaptionFontSize": "14",
                                "subcaptionFontBold": "0",
    ...
    CargaPeriodos: function () {
        if (Indicador3.colPeriodos.length < 1) {
            var url = contextPath + "Periodos/CargarPeriodos?idEstatus=5";
            $.getJSON(url, function (data) {
                Indicador3.colPeriodos = data;
                Indicador3.CargaListaPeriodos();
            }).fail(function () {
    ...
    CargaListaPeriodos: function () {
        var select = $(Indicador3.activeForm + ' #selPeriodos').empty();

        $.each(Indicador3.colPeriodos, function (i, item) {
    ...
    CargaListaRegiones: function () {
        var select = $(Indicador3.activeForm + ' #selRegiones').empty();

        $.each(Indicador3.colRegiones, function (i, item) {
            select.append('<option value="' + item.id + '">' + item.nombre + '</option>');
        });
    ...
    CargaListaZonas: function () {
        var select = $(Indicador3.activeForm + ' #selZonas').empty();

        $.each(Indicador3.colZonas, function (i, item) {
            select.append('<option value="' + item.id + '">' + item.nombre + '</option>');
        });
